 ...
 */
    ...
    $PAGE->set_url('/blocks/oc_mooc_nav/forum_view.php', $params);
...
/// Some capability checks.
	...
	global $USER, $DB;
		...
		if (count($oc_foren) > 1 and $oc_showall == '') {
			echo '<h2>'.get_string('all_forums', 'block_oc_mooc_nav').'</h2>';
			foreach ($oc_foren as $oc_forum) {
				...
				$oc_link = html_writer::link(new moodle_url('/blocks/oc_mooc_nav/forum_view.php?showall=false&id='.$oc_cm->id), $oc_forum->name);
				if ($oc_cm->visible == 1) {
...
/// Print settings and things across the top
	...
	// oncampus ////////////////
	require_once($CFG->dirroot.'/blocks/oc_mooc_nav/forum_lib.php');
